...
[![Tests](https://img.shields.io/badge/tests-232%20passing-22c55e)](#)
[![Lint](https://img.shields.io/badge/lint-0%20warnings-22c55e)](#)
[![Bundle](https://img.shields.io/badge/bundle-97KB%20gzip-22c55e)](#)
...
워드마크의 **`oo`**를 F1 레이싱 레드(`#E10600`)로 강조해 '실시간 온보드'를 표현했습니다. 타이포그래피는 **Formula1 공식 폰트**(self-host) + 한글 폴백 **Orbit**(Google Fonts).
...
- **Tailwind CSS 3** (다크 모드, flat 테마, 커스텀 토큰 — racing red / tire-* 팔레트)
- **Zustand 4** — 글로벌 타이머 + 텔레메트리 + 리더보드 + race control 상태
- **ml-matrix** — 텔레메트리 ↔ SVG affine fit
- **타이포그래피** — Formula1 5종 (FontFace API + ESM `?url`, prod-safe) + Orbit Google Fonts 한글 폴백
...
| **lerp + snap + extrap** | `trackLength/30` 이내 세그먼트는 선형 보간, 초과 시 텔레포트로 간주해 스냅. 마지막 샘플 뒤 2초까지 마지막 벡터로 외삽 (old_project 학습 그대로). |
| **Aspect-aware viewBox** | `ResizeObserver`가 컨테이너 종횡비를 측정 → viewBox를 비대칭 패딩으로 확장. `preserveAspectRatio="xMidYMid meet"`로 letterbox 없이 양축 채움. |
...
| **Race Info Panel** | `race_control` 폴링 → flag / SC / VSC 배지 + 24h 타임스탬프 메시지 리스트. 버퍼 50개 캡. |
| **Pirelli compound chip** | 리더보드 타이어 컬럼: 컴파운드 짧은 라벨(SOFT/MEDIUM/HARD/INTER/WET) + 공식 브로드캐스트 색상. |
...
npm test                    # Vitest 단위/통합 (232 tests)
...
src/
  ...
  store/       Zustand: timeline, session, telemetry, leaderboard, raceControl
  scheduler/   Poller (M1 LOCKED cadence, M2 pause/resume), interpolator (lerp + snap + extrap)
  utils/       affine fit, viewBox(aspect-aware), polyline smoothing, clean-lap picker, time
  components/
    Map/         CircuitMap (ResizeObserver), Marker, DecorationLayer
    Leaderboard/ Row, TireChip + tireColors (Pirelli palette), Sparkline
    Panels/      RaceInfoPanel (flag / SC / VSC / message list)
    Playback/    PlaybackBar, Calendar, SessionPicker, Scrubber, SpeedToggle, LiveDot
    Shell/       AppShell (responsive grid, flat 테마), Header, Wordmark
  hooks/       useMasterRaf (setTrackLength), useFrameBudget, useDriverMarker, useGlobalClock
  styles/      global.css (Orbit @import), fonts.ts (FontFace.load + ESM `?url`)
  assets/
    circuits/  40 SVG outlines (bacinger MIT) + circuits.json
    fonts/     Formula1 5종 TTF (self-host, prod-hashed)
